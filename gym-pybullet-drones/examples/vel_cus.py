...
def run(drone=DEFAULT_DRONE,
    ...
    INIT_XYZS = np.array([[0, 2*i, .1] for i in range(num_drones)])
    ...
    for i in range(NUM_WP):
        for j in range(num_drones):
            TARGET_VEL[j, i, :] = [.1, 0, 0, .5]  # [drone, wp, state] state[vx, vy, vz, 0.99(?)]
    ...
    #### Run the simulation ####################################
    action = np.zeros((num_drones,4))  # initial action (none)
    START = time.time()
